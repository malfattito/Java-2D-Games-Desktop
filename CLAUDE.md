...
**Layers**: tilemaps. `JGLayer` is an abstract base holding the tileset, the block map and the scroll state; the projection lives in the subclasses — `JGOrthoLayer` (orthogonal grid), `JGIsoLayer` (isometric diamonds) and `JGTopDownLayer` (top view with perspective and stacked blocks). Create them through `JGLevel.createOrthoLayer(...)` / `createIsoLayer(...)` / `createTopDownLayer(...)`; `vetLayers` is typed on the base so a scene can mix them. The interesting constructor path builds the map from an indexed *bitmap*: each pixel color in a `lay_*.bmp` is mapped through a `JGColorIndex[]` (color → tile frame index) onto tiles cut from a tile sheet. `speed` + `scrollLayer()` give the scrolling background, and every projection repeats the map indefinitely in every direction. Only `JGOrthoLayer` is used by the sample game; the other two are engine capability with no scene of their own.
...
`JGTopDownLayer` is the top view with perspective of the first GTA. Climbing a floor moves a block away from the camera, and that is a **scaling about the camera, not a shove**: what rises also grows on screen. Drawing the top the size of the base opens, between two neighbouring cells of one building, a gap as wide as they drew apart — and that gap is the ground showing through the building. A wall face is really a trapezoid, which an affine transform cannot draw. Each floor is drawn as a parallelogram and **the whole side is clipped to the true trapezoid**, so whatever a floor spills is trimmed and the base of a wall is a clean straight edge instead of a row of teeth. One clip per side of a building, and the frame of the fullest city costs 6.3 ms of the 33 it has. The floor is the same grid as the orthogonal layer, plus a height map: `createHeightMap(url, colors)` reads how many blocks are stacked on each cell, using the colour index as the height. The camera looks straight down at one screen point (`setCameraCenter`, screen middle by default), so a building is displaced away from that point by `perspective * distance` per floor — over the camera only roofs show, and the walls open outwards towards the edges. Only the faces turned to the camera are painted, one quad per floor, and a face shared with a neighbour of the same height is skipped, so a wide block reads as one building instead of several boxes. The wall tile comes from `setWallFrameIndex(wall)` for everything, or `setWallFrameIndex(roof, wall)` per kind of building — the roof on the floor map says which building it is, so one city mixes walls of several colours and window shapes without a second map; the roof reuses the tile the cell has on the floor map. Buildings are drawn from the farthest to the nearest so a near one can cover a far one. `getHeightAt()` / `isWallAt()` answer whether a screen point is inside a building.

**Texts** (`JGFont`): a line of text as an object — typeface, size, colour, `alignment` (`LEFT`/`CENTER`/`RIGHT`), `visible`, and a `position` whose *y* is the baseline. Create them through `JGLevel.createFont(family, style, size)`; the level keeps them in `vetTexts` and **renders them after the layers and the sprites**, so a text is never covered by the scene. `getWidth()`/`getLeft()`/`getRight()` measure against the current `Graphics2D` (zero before the window exists), which is what lets a scene place something beside a word without measuring by hand — `centerOnScreen(y)` is the common case. `render()` restores the font of the context afterwards, so scenes that still draw by hand around it are unaffected. Centring rounds *down*, matching `(width - textWidth) / 2`.
...
**Ground painter**: `JGTopDownLayer.setGroundPainter(p)` hands the layer something to draw on the ground, called after the ground tiles and before the buildings and actors. It is the seam a mark that belongs to the floor needs — a target, a shadow, a skid — so that everything standing on the floor passes over it instead of the mark being pasted on top of the scene.

**Frame rate and fonts**: `JGEngine.setFrameRate(fps)` sets the loop's frame time (30 by default); everything in the engine and in a game written on it runs on `JGTimeManager` and `JGTimer` time, so the rate is a choice of smoothness against work per second, not a change of behaviour. `JGWindowManager.warmFonts(families...)` measures and draws each family once into the back buffer before the window is shown: the first use of a font in Java2D costs a fraction of a second, once per process, and without the warm-up the first frame of the first screen to use a family paid it, 150 to 300 ms.

**Audio**: `JGSoundEffect` holds a short WAV, or a track meant to loop, as samples in memory in one format, 16-bit stereo 44100 Hz, converted at load (mono is widened, another rate resampled by linear interpolation). Playing goes through `JGAudioMixer`: one `SourceDataLine` opened and started once, when the first sound loads, fed by a daemon thread that sums every voice sounding at the moment into 512-frame blocks over a 2048-frame line buffer, so a sound is heard at most 46 ms after `play()`. `play()` adds a voice (six per effect, the oldest giving way past that, 32 in all), `stop()` removes them, `loop()` writes the samples back to back, sample exact, which is what music needs and what `JGMusic` cannot do, since it reopens the stream to start over. All three return at once, and that is why the mixer exists: the engine used to open a `javax.sound` `Clip` per copy of each sound, and on macOS starting or stopping a Clip costs the calling thread 120 to 250 ms every time, for every clip, whether or not the device is already running — a game thread froze at every effect and at every change of screen. `JGSoundEffect.setRate(r)` is the pitch, the step at which the mixer reads the samples, and it applies to what is already sounding: 1 is the file as written, 2 an octave up. A voice walks its samples in frames with a decimal, taking each output sample from the line between the two frames it falls between, which is what lets the pitch move without the grain of repeating and skipping samples. Past about twice the written pitch a sample thins out, so the useful range is narrow. `JGSoundManager.loadTrack(url)` gives the same object with a **single** voice, for a long sound meant to loop; the cost is the whole track as PCM in memory, 3 MB for twenty seconds. `JGSoundManager.free()` closes the mixer. `JGMusic` wraps the JavaZOOM BasicPlayer stack (the jars in `Libs/`) for streamed MP3/OGG, with an iOS-`AVAudioPlayer`-shaped API (`setNumberOfLoops(-1)` = loop forever).
