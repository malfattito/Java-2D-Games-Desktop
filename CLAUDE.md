...
**Audio**: `JGSoundEffect` wraps `javax.sound.sampled` for short WAVs, with six copies of the sound so repeated shots overlap instead of cutting each other, handed out in turn; `loop()` plays the first copy for as long as it is not stopped. `JGSoundManager.loadTrack(url)` loads the same object with a **single** copy, for a long sound meant to loop — a track never overlaps itself, and six copies of a whole song would be six times the file in memory. That path also loops **sample exact**, which is what music needs and what `JGMusic` cannot do, since it reopens the stream to start over. The cost is the whole track as PCM in memory, so it is WAV against a compressed file: 3 MB for twenty seconds. `JGMusic` wraps the JavaZOOM BasicPlayer stack (the jars in `Libs/`) for streamed MP3/OGG, with an iOS-`AVAudioPlayer`-shaped API (`setNumberOfLoops(-1)` = loop forever).
